<!--
 ...
 <script>
    ...
    $: {
      const yRange = !flipAtZero ? $yScale.range()[0] : $yScale(0);
      area = path + (
        'L' + $xScale($extents.x ? $extents.x[1] : 0) + ',' + yRange +
        'L' + $xScale($extents.x ? $extents.x[0] : 0) + ',' + yRange +
        'Z'
      );
    }
  </script>
